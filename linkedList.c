...
Node *head = NULL; // first pointer that points to the list

// add node to the list
// returns a node pointer to the node that it returns
Node *addNode(int data)
{
	...
	if (head == NULL)
	{
		...
		new->data = data; // new is a Node struct with data variable. -> references the data variable in new
		head = new;
		new->next = NULL;
	} else{
	// 2. Not empty list
...
void printMenu()
{
	...
	printf("\t3. Insert node into a position in the list\n");
	printf("\t4. Print list\n");
	printf("\t5. Quit.\n");
	return;
}
		...
		if (num_received == 1 && option > 0 && option <= 5)
		{
			switch (option)
			{
				...
				case 3: 
					...
					scanf("%d", &arg1);
					printf("What position should I insert?\n");
					scanf("%d", &arg2);
